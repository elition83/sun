...
//        echo "\n🚀  {$model} -> {$model['fields']['table']}  🚀\n";
    echo "====================================\n";
    //generateModel($model, $fields);
    
    //generateMigration($model, $fields, $fields['table']);
...
function generateSchemaFields($modelData)
{
    ...
    // Добавляем ID только для НЕ пивотных таблиц
    if (!$isPivot) {
        $result .= "            \$table->id();\n";
    }
...
function generateSchemaFields1($model, $fields)
{
    $result = "";
    $result .= "            \$table->id();\n"; // ID первичный ключ
...
function parseFieldDefinition($name, $definitions)
{
    ...
    foreach ($definitions as $definition) {
        if (strpos($definition, 'foreignId') !== false) {
            $field .= "\$table->$definition";
        } elseif (strpos($definition, 'foreign') !== false) {
            $field .= "\$table->$definition";
        } else {
            // Разбираем `string(255)`, `integer`, `boolean` и т.д.
            if (strpos($definition, '(') !== false) {
                list($type, $params) = explode('(', $definition);
                $params = rtrim($params, ')');
                $field .= "\$table->$type('$name', $params)";
            } else {
                $field .= "\$table->$definition('$name')";
            }
        }
    }
...
function parseFieldDefinition1($name, $definition)
{
...
function replaceCodeInFile($filePath, $search, $putCode)
{
    ...
    $indentation = substr($code, $lineStart, $braceOpenPos - $lineStart);
    ...
    // Формируем новое содержимое с сохранением отступов
    $newCode = substr($code, 0, $braceOpenPos + 1) . "\n" . $indentation . "    " . $putCode . "\n" . substr($code, $pos - 1);
...
function generateMigration1($model, $fields, $table) {
    $lowtable=strtolower($table);
